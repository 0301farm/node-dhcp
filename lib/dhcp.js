...
function Server(config, listenOnly) {
  ...
  var tftpServer = tftp.createServer({
    ...
    root: path.join(__dirname, "../", "tftp"),
    denyPUT: true
  });

  tftpServer.on('listening', function () {
    console.log('TFTP server listening on ' + config.server + ':' + (config.tftpPort || 69))
  });
...
Server.prototype = {
  ...
  config: function(key) {
    ...
    if (val instanceof Function && !Options.opts[optId].useCallback) {
      val = val.call(this);
    }
  ...
  _getOptions: function(mac, pre, required, requested, callback) {
    ...
    for (var req of required) {
      ...
      if (Options.opts[req] !== undefined) {
        ...
        if (pre[req] === undefined) {
          ...
          if (val instanceof Function) { // uses callback
            var copyReq = req;
            var copyVal = val;
            callbacksList.push(function (mac, done) {
              copyVal(mac, function (response) {
                if (response) {
                  pre[copyReq] = response;
                }
    ...
    if (callbacksList.length > 0) {
      var done = _.after(callbacksList.length, function () {
        return callback(pre);
      });

      _.each(callbacksList, function (f) {
        f(mac, done);
      });
    } else {
      return callback(pre);
  ...
  _selectAddress: function(clientMAC, callback) {
    ...
    //  return callback(this._state[clientMAC].address);
    // }
    ...
    if (static[clientMAC]) {
      return callback(static[clientMAC]);
    }

    var _self = this;

    var staticFunction = this._conf['staticFunction'];
    if (typeof staticFunction === 'function') {
      return staticFunction(clientMAC, function (address) {
        if (address) {
          return callback(address);
        }

        var randIP = _self.config('randomIP');
        var _tmp = _self.config('range');
        ...
        var ips = [_self.config('server')]; // Exclude our own server IP from pool
        var oldestMac = null;
        var oldestTime = Infinity;
        var leases = 0;
        for (var mac in _self._state) {
          if (_self._state[mac].address)
            ips.push(_self._state[mac].address);

          if (_self._state[mac].leaseTime < oldestTime) {
            oldestTime = _self._state[mac].leaseTime;
            oldestMac = mac;
          }
          leases++;
        }
        ...
        if (lastIP - firstIP === leases) {
          var ip = _self._state[oldestMac].address;

          // TODO: Notify deleted client
          delete _self._state[oldestMac];
  ...
  handleDiscover: function(req) {
    ...
    this._selectAddress(req.chaddr, cb);
  ...
  sendOffer: function(req) {
    ...
    var send = _.after(2, function () {
      // Send the actual data
      // INADDR_BROADCAST : 68 <- SERVER_IP : 67
      _self._send(INADDR_BROADCAST, ans);
    })

    function getOptionsCallback (pre) {
      if (req.options['77'] !== '105.80.88.69') { // iPXE ascii
        pre['67'] = 'undionly.kpxe';
      }
    ...
    function selectAddrCallback (address) {
      ans.yiaddr = address; // My offer
      send();      
    }

    this._selectAddress(req.chaddr, selectAddrCallback);
    this._getOptions(req.chaddr, {
        53: DHCPOFFER
      }, [
        1, 3, 51, 54, 6, 67
      ], req.options[55], getOptionsCallback);
  },
